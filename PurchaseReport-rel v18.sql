 ...
 * Unresolved issues:
1.  If I get rid of the NOT LIKE @SMCKod selector (which is not really needed), the runtime goes mental when I exclude duplicate SMCs (ones 
    where the SMC is listed in its own cat number set)
2.  If i use RelevantSMC to seed the big sql it hangs
 */
 ...
 *
 * Release History:
 * VERSION 18:  17.11.2014 Added linked SMC pricing - min price column and proveri column
...
CREATE CLUSTERED INDEX sgIdxRelevant ON #sgRelevantSMC(SMCRaw);
     ...
     FROM
        ...
        FULL OUTER JOIN
            ...
            FROM
                ...
                WHERE K.Kod1 NOT LIKE @SMCKod AND K.Aktivno = 1) Linked ON Linked.SvurzanSMC = Grupa.CatalozhenNomer
...
SELECT Active.GroupSMCKod AS 'SMC ot Grupa',
	   ...
	   Proveri = CASE WHEN (Active.LinkedSMC IS NULL AND Q.Quality IS NOT NULL AND ISNULL(SofiaM.SofiaMax,0) = 999999 AND Cheapest.MinPrice < Active.PurchasePriceBGN)
                        OR (Active.LinkedSMC IS NULL AND Q.Quality IS NOT NULL AND ISNULL(SofiaM.SofiaMax,0) = 0 AND Cheapest.MinPrice > Active.PurchasePriceBGN) THEN 1
                      ELSE 0 END,
...
SELECT K.Kod1 AS GroupSMCKod, NULL AS LinkedSMC,K.ID AS SMCRaw,K.Ime1 AS GroupSMCIme, K.CenaO1 AS PurchasePriceBGN,ISNULL(LT.LeadTime,1) AS LeadTimeInMonths
            FROM T2014.dbo.[Klas_SMC] K
            LEFT JOIN T2012.dbo.sgLeadTimes LT ON LT.Grupa = K.Grupi
            WHERE K.Kod1 LIKE @SMCKod AND K.Aktivno = 1) Active
LEFT JOIN
(SELECT MinPrices.GroupSMCKod,MIN(PurchasePriceBGN) AS MinPrice FROM
...
--DEV SNIP TO HERE	   
ORDER BY Active.GroupSMCKod,Active.LinkedSMC;
/*
...
BEGIN
	...
	SELECT @firstOrderValue = SUM([Posl. Dost. Cena bez DDS v lv]*[CHISTA ZAIAVKA]) FROM #sgAdjPurchaseReport Report WHERE Report.LinkedSMC IS NULL; 
	...
	SELECT @newOrderValue = SUM([Posl. Dost. Cena bez DDS v lv]*[CHISTA ZAIAVKA]) FROM #sgAdjPurchaseReport Report WHERE Report.LinkedSMC IS NULL;
...
BEGIN
    ...
    SELECT @orderValue = SUM([Posl. Dost. Cena bez DDS v lv]*[CHISTA ZAIAVKA]) FROM #sgAdjPurchaseReport Report WHERE Report.LinkedSMC IS NULL
--	DECLARE @GroupChar VARCHAR(50);
--SELECT TOP 1 @GroupChar = G.Ime
--FROM #sgAdjPurchaseReport PR
--LEFT JOIN dbo.Klas_SMC K ON K.Kod1 = PR.[SMC ot Grupa]
--LEFT JOIN dbo.Grupi_SMC G ON G.Code = K.Grupi
--WHERE PR.[LinkedSMC] IS NULL AND K.Grupi IS NOT NULL;	
